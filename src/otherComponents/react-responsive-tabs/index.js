...
export default class Tabs extends Component {
  ...
  setTabsDimensions = () => {
    ...
    Object.keys(this.tabRefs).forEach(key => {
      if (this.tabRefs[key]) {
        const width = this.tabRefs[key].tab.offsetWidth + 6;
  ...
  getTabs = () => {
    const { showMore, transform, transformWidth, items, allowRemove, removeActiveOnly, onRemove,onChildrenClick } = this.props;
  ...
  getTabProps = ({ title, key, selected, collapsed, tabIndex, disabled, className, onRemove, allowRemove,onChildrenClick }) => ({
    selected,
    allowRemove,
    children: title,
    key: tabPrefix + key,
        ...
        {() => (
          <div className={containerClasses} ref={this.tabsWrapper} onKeyDown={this.onKeyDown}>
            <div className={tabsClasses}>
              ...
              {!isCollapsed && (
                <ShowMore count={tabsHidden.length} {...this.getShowMoreProps(showMore, isSelectedTabHidden, showMoreLabel)}>
                  {tabsHidden.map(tab => (
            ...
            {showPanel && this.getExpandedTabs(panels, selectedTabKey, isCollapsed)}
